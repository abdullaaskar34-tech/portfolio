...
export default function SectionTitle({ icon: Icon, eyebrow, title, large = false, animate = false }) {
  const titleClass = `${large ? "text-4xl" : "text-3xl"} font-black tracking-tight text-slate-800 transition-all`;
  ...
  return (
    <div className={`${large ? "mb-10" : "mb-8"} flex items-center gap-6`}>
      ...
      <div>
        ...
        {animate ? (
          <ScrollFloat textClassName={titleClass}>
            {title}
          </ScrollFloat>
        ) : (
